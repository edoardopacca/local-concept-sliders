...
#     "Transformer2DModel",  # どうやらこっちの方らしい？ # attn1, 2
    "Attention",
    # Flux MMDiT (diffusers 0.35+): la attention di Flux ha una classe
    # dedicata invece di usare la Attention generica.
    "FluxAttention",
    # Qwen/SD3/Lumina/future MMDiT varianti (safety net)
    "JointAttention",
]
...
def load_ortho_dict(n):
    # Fix tilde expansion (os.path.isfile/torch.save NON espandono '~')
    # + crea la cartella se non esiste (primo run).
...
class LoRANetwork(nn.Module):
    ...
    def create_modules(
        ...
        # DIAGNOSTIC: enumera tutte le classi di moduli con "attn" nel nome
        # per capire se target_replace_modules matcha la class reale.
        attn_classes_seen = {}
        for _n, _m in root_module.named_modules():
            if "attn" in _n:
                cls = _m.__class__.__name__
                attn_classes_seen[cls] = attn_classes_seen.get(cls, 0) + 1
        print(f"[lora-diag] attention classes con 'attn' nel name (top 10): "
              f"{sorted(attn_classes_seen.items(), key=lambda x: -x[1])[:10]}")
        print(f"[lora-diag] target_replace_modules attivi: {target_replace_modules}")
