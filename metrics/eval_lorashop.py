#!/usr/bin/env python3
"""
eval_lorashop.py — localization metrics for LoRAShop eval runs.

Reads eval runs generated by 01_generate_eval_runs.py and computes the same
localization metrics used in eval_selectivity_loc.py / eval_masked.py:

  inside  = mask_target region
  outside = complement of mask_target

Metrics per run × scale:
...
Output: metrics/results_flux_lorashop/{concept}/eval_results.csv + eval_aggregate.json

Usage:
  python metrics/eval_lorashop.py --concept smile_woman --device cuda
"""
import argparse
import csv
import json
...
CONCEPT_EDIT_PROMPT: Dict[str, str] = {
    "smile_woman":   "a woman smiling with a big smile",
    "curlyhair_man": "a man with curly hair, curly, wavy hair",
    "age_woman":     "an elderly woman with wrinkles, gray hair, aged skin",
    "furlength_dog": "a dog with long fur, fluffy coat, long-haired",
}

DEFAULT_RUNS_ROOT  = "flux/tasks/shop_concept/runs"
DEFAULT_OUTPUT_DIR = "metrics/results_flux_lorashop"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Localization metrics for LoRAShop eval runs."
    )
...
def compute_loc_metrics(
    ...
    mask_outside_np = 1.0 - mask_inside_np
    ...
    clip_loc = 0.0 if delta_in <= 0 else delta_in / (delta_in + abs(delta_out) + 1e-8)
...
def main() -> None:
    ...
    run_prefix = f"eval_{args.concept}_"
    runs = sorted(
    ...
    # Discover scales from files (fallback to EVAL_SCALES)
    scales = EVAL_SCALES
    for run_dir in runs:
        found = sorted(
            float(p.stem.split("_s")[-1])
            for p in run_dir.glob("edited_lorashop_s*.png")
        )
        if found:
            scales = found
            break

    print(f"[eval_lorashop]  concept={args.concept}  runs={len(runs)}  "
    ...
    for run_dir in runs:
        ...
        for scale in scales:
            edited_path = run_dir / f"edited_lorashop_s{scale:.1f}.png"
            if not edited_path.exists():
                print(f"  [skip] {run_dir.name}  s={scale:.1f}  (missing)")
                continue
